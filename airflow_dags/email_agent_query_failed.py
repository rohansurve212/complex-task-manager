...
default_args ={
    "owner":"eq02977",
    "description":"Notify user if the agent table query fails",
...
# Define the DAG
@dag (
    dag_id= "email_agent_query_failed",
    description = "Notify user if the agent table query fails",
...
def main_task():
    error = query_agent_table()
    send_email(error)

# Task to query the agent table
@task()
def query_agent_table():
...
# Task to send email if the agent table query fails
@task()
def send_email(error):
    ...
    if error:
        ...
        <h4>The agent table query failed because of the following error:</h4>
        <br><br>
        <h4>Error details: {} </h4>
        <br><br>
        ...
        email_file['Subject'] = "Agent Table Query failed -- EMAIL Alert"
