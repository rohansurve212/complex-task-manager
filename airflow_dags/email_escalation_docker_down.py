...
default_args ={
    "owner":"eq02977",
    "description":"Notify user if Escalation Docker Container is down",
...
# Define the DAG
@dag (
    dag_id= "email_escalation_docker_down",
    description = "Notify user if Escalation Docker Container is down",
...
def read_log_file():
    cont_down = False
    log_file_path = '/usr/local/airflow/files/stm_assets/Escalation/log/escalation_django.log'
        ...
        with open(log_file_path, 'r') as f:
            ...
            if lines:
                ...
                if "successfully" in last_line:
                    ...
                    if current_time - timestamp > timedelta(minutes=65):
                        cont_down = True 
                else:
                    cont_down = True # If the last line does not contain the log entry
            else:
                cont_down = True # If the log file is empty
...
def send_email(cont_down):
    ...
    if cont_down:
        ...
        <h4>Escalation Docker Container seems to be down. Please check the container logs.</h4>
        <br><br>
        ...
        email_file['Subject'] = "Escalation Docker Container Down -- EMAIL Alert"
