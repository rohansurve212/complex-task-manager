...
import pika, sys, os
import os
import parsing_disposition_codes
import time
import logging
import pyodbc
...
# File Handler
handler = FlushingTimedRotatingFileHandler('/var/log/disposition_codes.log',when="h",interval=1,backupCount=168)
...
def create_rmq_channel(rmq_conn_params, queue, sql_db_conn, retry_count_holder):
    ...
    def callback(ch, method, properties, body):
        try:
            ...
            parsing_disposition_codes.rmp_parsing(message_body, sql_db_conn)
    ...
    while True:
        try:
            ...
            if rmq_conn is None or not rmq_conn.is_open:
                    ...
                    rmq_conn, channel = create_rmq_channel(rmq_conn_params, 'Disposition codes', sql_db_conn, retry_count_holder)
