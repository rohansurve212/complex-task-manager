...
# File Handler
handler = FlushingTimedRotatingFileHandler('/var/log/agent_skills.log',when="midnight",interval=1,backupCount=7)
...
def create_rmq_channel(rmq_conn_params, queue, sql_db_conn, retry_count_holder):
    ...
    def callback(ch, method, properties, body):
        try:
            ...
            parsing_agent_skills.rmp_parsing(message_body, sql_db_conn)
    ...
    while True:
        try:
            ...
            if rmq_conn is None or not rmq_conn.is_open:
                    ...
                    rmq_conn, channel = create_rmq_channel(rmq_conn_params, 'Skillsets assignment', sql_db_conn, retry_count_holder)
